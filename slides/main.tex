...
\begin{frame}[fragile]{Inside SOR: the explanation-merge-tree}
  ...
  \begin{columns}[T]
    \begin{column}{0.20\textwidth}
      \vspace{14pt}
      \begin{card}[0.98\textwidth]
      \tiny
      \textbf{1.} Parse the SVA into a syntax tree

      \vspace{3pt}
      \textbf{2.} Merge the tree bottom-up into NL

      \vspace{3pt}
      \textbf{3.} Compare with the original ask above
      \end{card}
    \end{column}
    \begin{column}{0.80\textwidth}
      ...
      \begin{tikzpicture}[
          leaf/.style={rectangle, rounded corners, draw=nyuvioletlight, fill=white, text=cardtext, minimum width=1.6cm, minimum height=0.8cm, align=center, font=\scriptsize},
          ...
          lbl/.style={font=\scriptsize, text=nyumuted},
        ]
        \node[leaf] (la) at (-4.0,0) {\texttt{a}};
        \node[leaf] (lb) at (-2.2,0) {\texttt{b}};
        \node[leaf] (lc) at (2.7,0) {\texttt{c}};

        \node[merge] (mand) at (-2.75,1.5) {a and b are both asserted};
        \node[lbl, fill=white, inner sep=1pt] at (-3.5,0.75) {parse node: \texttt{\&\&}};
        \node[root] (root) at (0,3.1) {whenever a and b are both asserted, then c is asserted 2 cycles later};
        \node[lbl, fill=white, inner sep=1pt] at (-1.4,2.3) {parse node: \texttt{|-> \#\#2}};
  ...
  \begin{card}
  \tiny
  $\rightarrow$ \textbf{Mismatch found:} the tree's root says \textbf{2} cycles
  later, but the original ask above says \textbf{3} -- SOR flags just the
  \texttt{\#\#2} node and revises it to \texttt{\#\#3}, not the whole property.
  \end{card}
\end{frame}
...
\begin{frame}{The improvement: SOR-timing fixes vs.\ the original pipeline}
  ...
  \begin{tabular}{l l r}
    \toprule
    \textbf{Dataset} & \textbf{Configuration} & \textbf{FM-strict} \\
    \midrule
    \multirow{2}{*}{\shortstack[l]{nl2sva\_human\_\\verified (73)}}
      & Original pipeline & 71.2--74.0\% \\
      & \textbf{+ SOR-timing fixes} & \textbf{74.0\%} \\
    \midrule
    \multirow{2}{*}{\shortstack[l]{nl2sva\_machine\_\\verified (283)}}
      & Original pipeline & 82.0\% (232/283) \\
      & \textbf{+ SOR-timing fixes} & \textbf{85.2\% (241/283)} \\
    \bottomrule
  \end{tabular}}
  \end{card}
  \vspace{8pt}
  \begin{card}
  \scriptsize
  A separate, unrelated bug was also fixed along the way: range/unbounded
  delays (\texttt{\#\#[M:N]}, \texttt{\#\#[*]}, \texttt{\#\#[+]}) were
  losing their real bounds internally.
  \end{card}
\end{frame}
