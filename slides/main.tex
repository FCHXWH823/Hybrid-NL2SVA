...
\begin{frame}{Why NL2SVA is hard}
  ...
  \begin{itemize}
    ...
    \item SVA has timing words that look almost the same but mean
      ...
      \end{itemize}
    \item Input descriptions come at different levels of detail -- e.g.\
      \textit{``that ack correctly follows a request''} (vague) vs.\
      \textit{``whenever req rises, ack must be asserted exactly 1 clock
      cycle later''} (already precise and names real signals); vague ones
      need grounding first (Step 1, next slide)
  \end{itemize}
  \end{card}
\end{frame}
