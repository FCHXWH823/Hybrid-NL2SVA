...
\begin{frame}{Why NL2SVA is hard}
  ...
  \begin{columns}[T]
    \begin{column}{0.5\textwidth}
      \scriptsize\textbf{Why a general AI model struggles.}
      \begin{itemize}
    ...
    \end{column}
    \begin{column}{0.5\textwidth}
      \scriptsize\textbf{Our approach.} Never trust one AI answer -- three
      steps instead:
      \begin{itemize}
        \item \textbf{Step 1:} rewrite the input into OL-NL -- a precise,
          signal-grounded plain-English restatement
        \item \textbf{Step 2:} HybridRetrieval-Augmented Generation --
          give the model the right operator knowledge, then write a
          first answer
        \item \textbf{Step 3:} use JasperGold to fix syntax errors, and
          automatically rebuild the answer's meaning as a tree to catch
          and fix mistakes in what it actually checks
      \end{itemize}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}{Recap: the original three-stage pipeline}
  ...
  itself (\texttt{EXPRESSION\_ONLY\_INSTRUCTION}) --- these are added
  automatically (\texttt{wrap\_property\_expression}) once the bare boolean/temporal
...
\begin{frame}{Case study: when the pipeline does \emph{not} help}
  ...
  what. (nl2sva\_machine\_verified does \emph{not} show this wash for the
