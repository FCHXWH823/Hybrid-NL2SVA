    ...
    \begin{column}{0.5\textwidth}
      ...
      \begin{itemize}
        \item SVA has timing words that look almost the same but mean
          different things (\texttt{|->} vs.\ \texttt{|=>},
          \texttt{\#\#N} vs.\ \texttt{\#\#[M:N]})
        \item Easy to get the order of operations wrong (\texttt{and}/
          \texttt{or} bind tighter than \texttt{|->})
        \item Not much SVA code in the data these models learned from
      \end{itemize}
    \end{column}
    \begin{column}{0.5\textwidth}
      \scriptsize\textbf{Our approach.} Don't just ask a general AI model once:
      \begin{itemize}
        \item look up real, correct operator rules first
        \item write a first-try answer
        \item check and fix it with a real formal-verification tool
          (JasperGold) before trusting it
      \end{itemize}
    \end{column}
  \end{columns}
\end{frame}
