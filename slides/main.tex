...
% ------------------------------ NYU palette ---------------------------------
\definecolor{nyuviolet}{HTML}{57068C}      % NYU primary violet
\definecolor{nyuvioletdark}{HTML}{3C0561}  % darker shade, for gradients/shadows
\definecolor{nyuvioletlight}{HTML}{B185D6} % light accent (bullets, rules)
\definecolor{nyulavender}{HTML}{E9DCF5}    % pale panel background
...
\setbeamercolor{background canvas}{bg=nyuviolet}
...
\setbeamercolor{framesubtitle}{fg=nyuvioletlight}
\setbeamercolor{structure}{fg=nyuvioletlight}
\setbeamercolor{itemize item}{fg=nyuvioletlight}
\setbeamercolor{itemize subitem}{fg=nyuvioletlight}
\setbeamercolor{section in toc}{fg=white}
\setbeamercolor{subsection in toc}{fg=nyuvioletlight}
\setbeamercolor{page number in head/foot}{fg=nyuvioletlight}
...
\setbeamertemplate{footline}{
  ...
  \hbox{\begin{beamercolorbox}[wd=\paperwidth,ht=2.4ex,dp=1ex,left,leftskip=1em,right,rightskip=1em]{page number in head/foot}
    \textcolor{nyuvioletlight}{\scriptsize Hybrid-NL2SVA}
    \hfill
    \textcolor{nyuvioletlight}{\scriptsize\insertframenumber\,/\,\inserttotalframenumber}
  \end{beamercolorbox}}
}

% A "card": white rounded panel for data-heavy content (tables, code) so it
% stays legible against the violet background. Needs its own named beamer
% color (bg=cardwhite/fg=cardtext) -- passing a raw xcolor name like "white"
% straight to beamercolorbox silently resolves to an undefined/transparent
% fill, which just left text sitting directly on the violet canvas.
\setbeamercolor{card}{bg=cardwhite, fg=cardtext}
...
\begin{frame}{Why NL2SVA is hard}
  \begin{columns}[T]
    \begin{column}{0.5\textwidth}
      \textbf{The task.} Given
      \begin{itemize}
        \item an RTL design (or testbench), and
        \item a natural-language verification property,
      \end{itemize}
      produce a SystemVerilog Assertion (SVA) that is \emph{functionally
      equivalent} to what a verification engineer intended.
    \end{column}
    \begin{column}{0.5\textwidth}
      \textbf{Why general-purpose LLMs struggle.}
      \begin{itemize}
        \item SVA has subtle, easy-to-confuse temporal operators
          (\texttt{|->} vs.\ \texttt{|=>}, \texttt{\#\#N} vs.\ \texttt{\#\#[M:N]},
          \texttt{s\_eventually} vs.\ plain \texttt{eventually})
        \item Operator precedence traps (\texttt{and}/\texttt{or} binding
          tighter than \texttt{|->})
        \item Little SVA-specific data in general pretraining corpora
      \end{itemize}
    \end{column}
  \end{columns}
  \vspace{6pt}
  \begin{card}
    \textbf{Our approach:} don't just prompt a general LLM once --
    retrieve grounded operator knowledge, generate, then \emph{mechanically
    verify and self-correct} against a real formal tool (JasperGold) before
    ever trusting the answer.
  \end{card}
\end{frame}

\section{Pipeline Architecture}

\begin{frame}{Recap: the original three-stage pipeline}
  \framesubtitle{The baseline this talk's improvements build on top of}
  \centering
  \resizebox{\textwidth}{!}{%
  \begin{tikzpicture}[
      stage/.style={rectangle, rounded corners, draw=white, fill=nyuvioletdark, text=white, minimum width=3.6cm, minimum height=1.5cm, align=center, font=\small},
      ...
      arrow/.style={-{Latex[length=2.5mm]}, thick, white},
    ]
    \node[stage] (s1) at (0,0) {\textbf{Step 1}\\OL-NL Grounding};
    \node[stage] (s2) at (5.2,0) {\textbf{Stage 2}\\HybridRetrieval-\\Augmented Generation};
    \node[stage] (s3) at (10.4,0) {\textbf{Stage 3}\\JG Syntax Cleanup\\+ SOR};
  ...
  \begin{card}
  \small
  The model is \emph{never} trusted to write the clock/\texttt{disable iff}/label
  itself (\texttt{EXPRESSION\_ONLY\_INSTRUCTION}) --- these are mechanically
  attached (\texttt{wrap\_property\_expression}) once the bare boolean/temporal
  expression is finalized.
  \end{card}
\end{frame}

\begin{frame}[fragile]{Stage 1 in detail: OL-NL grounding}
  \framesubtitle{\texttt{--ol-nl-grounding} -- only needed when the input isn't already signal-grounded}
  \begin{card}
  \scriptsize
  ``OL NL'' = \textbf{operator-level natural language}: a restatement naming
  \emph{only} signals that actually appear in the testbench (never invented/
  paraphrased names), stating the property's structure as precisely as
  possible -- condition first, then outcome. One best-effort LLM call, no
  golden SVA shown.
  \end{card}
  \vspace{4pt}
  \begin{card}
  ...
  \begin{verbatim}
Description: that the pending flag never gets stuck.
  Signals: pending, pending_d1, clear_vld, sat_guard
OL NL: It must never be the case that pending_d1 is asserted
and clear_vld is not asserted and pending is still asserted
and sat_guard is not asserted
  \end{verbatim}
  \end{card}
  \vspace{4pt}
  \begin{card}
  \scriptsize
  \textbf{When it runs:} needed for \texttt{nl2sva\_human\_verified}; \emph{
  redundant} for \texttt{nl2sva\_machine\_verified} (already names every
  signal) -- skipped there. \texttt{--ol-nl-conservative}: stay maximally
  literal, never invent structure the description doesn't state.
  \end{card}
\end{frame}

\begin{frame}{Stage 2 in detail: HybridRetrieval-augmented generation}
  \framesubtitle{Two independent retrieval paths feed one generation call}
  \centering
  \resizebox{0.72\textwidth}{!}{%
  \begin{tikzpicture}[
      node/.style={rectangle, rounded corners, draw=white, fill=nyuvioletdark, text=white, minimum width=4.4cm, minimum height=1cm, align=center, font=\small},
      wide/.style={rectangle, rounded corners, draw=white, fill=goodgreen!70!nyuvioletdark, text=white, minimum width=4.4cm, minimum height=1cm, align=center, font=\small},
      arrow/.style={-{Latex[length=2.5mm]}, thick, white},
      note/.style={font=\scriptsize, text=nyuvioletlight, align=center},
    ]
    \node[node] (in) at (0,1.6) {OL-NL grounded property\\+ RTL testbench};
    \node[node] (kw) at (-3.6,0) {Keyword/operator-guided\\retrieval};
    \node[node] (sem) at (3.6,0) {General-semantic\\retrieval};
    \node[wide] (gen) at (0,-1.6) {Single completion\\$\rightarrow$ bare property expression};
    ...
    \node[note] at (-3.6,-0.9) {\texttt{extract\_keywords} +\\46-entry operator table};
    \node[note] at (3.6,-0.9) {query on the raw\\NL prompt directly};
  \end{tikzpicture}}
  \vspace{6pt}
  \begin{card}
  \scriptsize
  Retrieves matching code-centric chunks from \textbf{10 SVA textbooks +
  IEEE~1800-2017's own Assertions chapter} (persisted Chroma vector DB),
  injected as grounding context. The model still outputs only the bare
  boolean/temporal expression -- \texttt{clk}/\texttt{disable iff}/label are
  never its job.
  \end{card}
\end{frame}
...
\begin{frame}{Stage 3 in detail: syntax cleanup + SOR}
  ...
  \begin{tikzpicture}[
      node/.style={rectangle, rounded corners, draw=white, fill=nyuvioletdark, text=white, minimum width=3.2cm, minimum height=1.1cm, align=center, font=\small},
      arrow/.style={-{Latex[length=2.5mm]}, thick, white},
      note/.style={font=\scriptsize, text=nyuvioletlight, align=center},
    ]
...
Candidate SVA:  a && b |-> ##2 c
  ...
  \begin{tikzpicture}[
      ...
      merge/.style={rectangle, rounded corners, draw=white, fill=nyuvioletdark, text=white, minimum width=6.4cm, minimum height=0.8cm, align=center, font=\scriptsize},
      ...
      arrow/.style={-{Latex[length=2mm]}, thick, nyuvioletlight},
    ]
...
\begin{frame}{The bug this talk's improvement targets}
  \framesubtitle{A confirmed, systematic timing error in \texttt{|=>} + \texttt{\#\#N} composition}
  \begin{card}
  \scriptsize
  Composing \texttt{|=>} (which itself already advances one clock cycle)
  with an explicit \texttt{\#\#N} delay silently lands on \texttt{N+1} total
  ...
  \begin{card}
  \scriptsize
  \textbf{Fix (\texttt{--only-overlap-implication}):} sidestep it at the
  root -- always use \texttt{|->}, express any delay explicitly via
  \texttt{\#\#N} using the \emph{total} cycle count, never combine an
  implicit-advance operator with an explicit one.
  \end{card}
  \vspace{5pt}
  \begin{card}
  \tiny
  \texttt{--sor-template-timing}: SOR's explanation-merge-tree can also use
  fixed, deterministic, LLM-free templates (\texttt{sva\_temporal\_
...
\begin{frame}{Cross-model comparison --- RAG+SOR pipeline}
  ...
  \end{card}
  \vspace{4pt}
  {\scriptsize\textcolor{nyuvioletlight}{DeepSeek-V4-Flash (thinking) was originally run and labeled ``DeepSeek-R1''
...
\begin{frame}{Case study: when the pipeline does \emph{not} help}
  ...
  \begin{columns}[T]
    ...
    \begin{column}{0.45\textwidth}
      \textbf{Three regressions found:}
      \begin{itemize}
        \item Spurious \texttt{\#\#N} inserted where none was needed
  ...
  during revision -- not a property that holds unconditionally across
  models. (nl2sva\_machine\_verified does \emph{not} show this wash for the
  same model: 60.8\% vs.\ 51.2\% FM-strict.)
  \end{card}
\end{frame}
...
\begin{frame}{From curated benchmarks to real designs}
  \begin{columns}[T]
    \begin{column}{0.5\textwidth}
      \textbf{What Part I evaluated:} \\
      NL2SVA on FVEval-Verified -- properties and RTL are already curated,
      one property per row, always paired with a golden SVA.
    \end{column}
    \begin{column}{0.5\textwidth}
...
\begin{frame}{Spec2NL + NL2SVA = end-to-end Spec2SVA}
  ...
  \resizebox{\textwidth}{!}{%
  \begin{tikzpicture}[
      big/.style={rectangle, rounded corners, draw=white, very thick, minimum width=5.2cm, minimum height=2.6cm, align=center},
      ...
      arrow/.style={-{Latex[length=3mm]}, ultra thick, white},
    ]
    \node[big, minimum height=3.2cm, fill=nyuvioletdark] (af) at (0,0) {};
    \node[align=center, text=white, font=\bfseries] at (0,1.45) {AssertionForge\\(Spec2NL)};
    ...
    \node[big, minimum height=3.2cm, fill=goodgreen!70!nyuvioletdark] (hn) at (8,0) {};
    \node[align=center, text=white, font=\bfseries] at (8,1.45) {Hybrid-NL2SVA\\(NL2SVA)};
    ...
    \node[rectangle, rounded corners, draw=white, fill=warnamber, text=white, minimum width=2.8cm, minimum height=1.3cm, align=center, font=\small] (jg) at (13.6,0) {JasperGold\\formal\\verification};

    \draw[arrow] (af) -- (hn);
    \draw[arrow] (hn) -- (jg);

    \node[font=\scriptsize, text=nyuvioletlight, align=center] at (4,-2.2) {NL property\\(per signal)};
    \node[font=\scriptsize, text=nyuvioletlight, align=center] at (11,-2.2) {candidate\\SVA};
  \end{tikzpicture}}
  \vspace{10pt}
  \begin{card}
    \footnotesize
...
\begin{frame}{UART: full NL2SVA + formal verification results}
  ...
  \begin{card}
    ...
    only evidence our pipeline lands in a plausible, self-consistent range.
...
\begin{frame}{A methodology gap: properties about free inputs}
  ...
  \begin{card}
    ...
    regardless of NL2SVA quality. Excluding these 4 signals lifts \#Proven
...
\begin{frame}{A second gap: signal hallucination, not just free inputs}
  ...
  \begin{card}
    ...
    just faithfully transcribed \texttt{baud\_rate}/\texttt{global\_clock\_freq}
...
//   baud_limit = (global_clock_freq / gcd(...)) - baud_freq
  ...
  \begin{card}
    ...
    revisions -- consistent with Part I's negative-priming finding: naming
    the exact bad pattern put that pattern in front of the model right
    before generation. Invalid-signal leakage rose
    14\%$\to$22\%$\to$27\%$\to$30\% across successive attempts to ban it in
    words.
  \end{card}
\end{frame}
...
\begin{frame}{Fix: two-step generation + a mechanical filter}
  ...
  \begin{itemize}
    ...
    \item \textbf{Mechanical filter (post-hoc, not a prompt):} drops any
      ...
      in \texttt{valid\_signals} -- carries none of the negative-priming risk
      above, since it never goes back into a prompt
  \end{itemize}
  \end{card}
  \vspace{8pt}
  \begin{card}
    \footnotesize
    \textbf{Lesson:} banning a specific bad pattern \emph{in words} tends to
    backfire; pushing the same constraint \emph{out of the prompt} into a
    mechanical check is what actually held.
  \end{card}
\end{frame}

\begin{frame}{Validated: same 44 properties, ungrounded vs.\ grounded}
  \framesubtitle{\texttt{baud\_clk}/\texttt{baud\_freq} pilot -- identical underlying ideas, index-matched}
  \begin{card}
  \centering
  \renewcommand{\arraystretch}{1.3}
  \resizebox{\linewidth}{!}{%
  ...
  \begin{card}
    \scriptsize
    Same 44 properties both ways (index-matched) -- grounding + the full
    pipeline is \textbf{+22.7pp \#SynC, +20.4pp \#Proven} over the best
    ungrounded baseline; a specialized NL2SVA model alone (CodeV-SVA)
    doesn't compensate for hallucinated-signal input the way grounding
    does. \textit{Scope: \texttt{baud\_clk}/\texttt{baud\_freq} only (2/18
    signals) -- pilot before the remaining 16 and a full regeneration.}
  \end{card}
\end{frame}
