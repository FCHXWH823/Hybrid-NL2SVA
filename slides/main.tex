...
\begin{frame}[fragile]{Inside SOR: the explanation-merge-tree}
  \framesubtitle{Merge bottom-up into NL, then compare against the original ask}
  \begin{card}
  \tiny
  \begin{verbatim}
Candidate SVA:  a && b |-> ##2 c
  ...
  \begin{tikzpicture}[
    ...
    \node[lbl, fill=white, inner sep=1pt] at (-3.5,0.75) {merge via \texttt{\&\&}};
    \node[root] (root) at (0,3.1) {whenever a and b are both asserted, then c is asserted 2 cycles later};
    \node[lbl, fill=white, inner sep=1pt] at (-1.4,2.3) {merge via \texttt{|-> \#\#2}};
  ...
  \begin{card}
  \tiny
  \textbf{From the SVA} (merged bottom-up): \textit{whenever a and b are
  both asserted, then c is asserted 2 cycles later}\\
  \textbf{OL-NL, Stage 1} (what was actually asked for): \textit{whenever a
  and b are both asserted, then c is asserted \textcolor{warnamber}{\textbf{3}}
  cycles later}\\
  $\rightarrow$ \textbf{Mismatch} on the delay -- SOR flags just the
  \texttt{\#\#2} node and revises it to \texttt{\#\#3}, not the whole
  property.
  \end{card}
\end{frame}
