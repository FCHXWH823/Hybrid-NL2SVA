...
\begin{frame}{The pipeline today: three stages}
  \framesubtitle{Stages 2 \& 3 are from the published MLCAD'25 paper; Step 1 was added since}
  \centering
  \resizebox{\textwidth}{!}{%
  \begin{tikzpicture}[
  ...
  \begin{card}
  \small
  The paper's own Fig.~2 only shows \textbf{Customized RAG} (dynamic
  splitting + HybridRetrieval) and \textbf{SOR} -- Step 1 (OL-NL grounding)
  is a later addition, only needed when the input isn't already
  signal-grounded. The model is \emph{never} trusted to write the
