...
\begin{frame}{Three-stage pipeline}
  \centering
  \resizebox{\textwidth}{!}{%
  \begin{tikzpicture}[
...
\begin{frame}{Stage 3 in detail: syntax cleanup + SOR}
  ...
  \begin{tikzpicture}[
    ...
    \node[note] at (4,-1.1) {Up to 3 rounds;\\real \texttt{jg} elaboration\\each round};
    \node[note] at (8,-1.1) {Explanation-merge-tree\\vs.\ OL-NL text;\\model flags mismatches};
    \node[note] at (12,-1.1) {SOR can itself\\introduce new\\elaboration errors};
  \end{tikzpicture}}
  \vspace{10pt}
  \begin{card}
  \footnotesize
...
\begin{frame}{HybridRetrieval}
  \begin{itemize}
    \item Splits the NL property description into operator-level phrases
    \item Maps each phrase to the most relevant SVA operator via a
      46-entry operator table (\texttt{sva\_temporal\_operators.json})
    \item Retrieves matching code-centric chunks from 10 SVA textbooks +
      IEEE~1800-2017's own Assertions chapter (persisted Chroma vector DB)
    \item Injected into Stage 2's generation prompt as grounding context
  \end{itemize}
\end{frame}

\begin{frame}{SOR: SVA Operator-based Rechecking}
  \begin{itemize}
    \item Builds an \textbf{explanation-merge-tree}: parses the candidate
      SVA's operator structure (\texttt{sva\_graph.py}), composes a
      natural-language explanation bottom-up
    \item Compares that composed explanation against the OL-NL grounded
      description
    \item If they disagree, the model revises the SVA -- but is instructed
      to change \emph{only} the minimal part responsible
      (\texttt{--sor-conservative})
  \end{itemize}
  \vspace{6pt}
  \begin{card}
    \footnotesize
    \textbf{A confirmed, systematic bug this design targets:} composing
    \texttt{|=>} (which itself advances one clock cycle) together with an
    explicit \texttt{\#\#N} delay silently lands on \texttt{N+1} total cycles
    instead of the intended \texttt{N}. \texttt{--only-overlap-implication}
    sidesteps it at the root: always use \texttt{|->}, express any delay
    explicitly via \texttt{\#\#N} using the \emph{total} cycle count.
  \end{card}
\end{frame}
