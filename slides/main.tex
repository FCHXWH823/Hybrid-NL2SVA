...
% A simple typographic NYU mark -- a bold "NYU" wordmark on a violet
% roundel, echoing NYU's own single-word violet lockup. Drawn locally
% (no official brand asset file was available to embed) rather than
% pulling an unverified copy from a third-party logo site.
\newcommand{\nyuemblem}[1][1]{%
  \begin{tikzpicture}[scale=#1]
    \node[circle, fill=white, minimum size=2.1cm] (c) at (0,0) {};
    \node[circle, draw=white, line width=1.2pt, minimum size=2.1cm] at (0,0) {};
    \node[text=nyuviolet, font=\bfseries\fontsize{22}{22}\selectfont] at (0,0) {NYU};
  \end{tikzpicture}%
}
...
\begin{frame}[plain]
  ...
  \begin{center}
    ...
    \vspace{14pt}
    {\usebeamerfont{title}\usebeamercolor[fg]{title}\Huge\bfseries\inserttitle}\\[10pt]
    {\usebeamerfont{subtitle}\color{nyuvioletlight}\large\insertsubtitle}\\[22pt]
    {\color{white}\insertauthor}\\[2pt]
    {\color{nyuvioletlight}\insertinstitute}\\[2pt]
    {\color{nyuvioletlight}\small\insertdate}
  \end{center}
\end{frame}
