...
\setbeamertemplate{footline}{
  \leavevmode
  \hbox{\begin{beamercolorbox}[wd=\paperwidth,ht=2.4ex,dp=1ex,right,rightskip=1em]{page number in head/foot}
...
\begin{frame}[plain]
  \begin{tikzpicture}[remember picture, overlay]
    \node[anchor=south west, xshift=0.6cm, yshift=0.4cm] at (current page.south west) {\includegraphics[width=1.3cm]{Figs/nyu_logo.pdf}};
  \end{tikzpicture}
  \vspace{6pt}
  \begin{center}
...
\begin{frame}{Our approach: three steps}
  ...
  \begin{card}
    \small
    \textbf{Step 1:} rewrite the input into \textbf{OL-NL} (operator-level
    natural language) -- name only real signals (e.g.\ \texttt{req},
    \texttt{ack}, \texttt{clk}, \texttt{rstn}), and describe how they
    relate in plain words that match one specific operator, e.g.\
    \textit{``whenever req rises, ack must be asserted exactly 1 clock
    cycle later''} matches \texttt{\$rose(req) |-> \#\#1 ack}, operator
    for operator, in plain English
  \end{card}
  \vspace{6pt}
  \begin{card}
...
\begin{frame}{The pipeline today: three stages}
  \framesubtitle{Step 1 and Stage 3's SOR are both new -- only Stage 2 is unchanged from the paper}
  \centering
  \resizebox{\textwidth}{!}{%
  \begin{tikzpicture}[
    ...
    \node[newstage] (s3) at (10.4,0) {\textbf{Stage 3 (new)}\\JG Syntax Cleanup\\+ SOR};
    ...
    \node[sub] (s3a) at (10.4,-1.9) {Real JG elaboration loop;\\\emph{syntax-tree-based} SOR\\(was flat operator lookup)};
  ...
  \begin{card}
  \small
  Step 1 is only needed when the input isn't already signal-grounded.
  Stage 3's SOR now parses the candidate SVA into a syntax tree and
  rebuilds its meaning bottom-up, replacing the paper's flat per-operator
  lookup.
  \end{card}
\end{frame}
...
\begin{frame}{Stage 3 / SOR: then vs.\ now}
  \centering
  \textbf{\small As published (MLCAD'25 paper):} flat, per-operator lookup

  \vspace{2pt}
  \includegraphics[width=0.3\textwidth]{Figs/SVAOperatorRefine.pdf}

  \vspace{10pt}
  \textbf{\small Today:} full Pre-SOR / SOR / Post-SOR pipeline, syntax-tree-based
  ...
  \begin{tikzpicture}[
    ...
    \node[node] (sor) at (8,0) {Syntax-tree-based\\SOR\\(functional recheck)};
    ...
    \node[note] at (8,-1.1) {Bottom-up explanation-\\merge-tree vs.\ OL-NL text};
...
\begin{frame}[fragile]{Inside SOR: the explanation-merge-tree}
  \framesubtitle{Parse to a syntax tree, merge bottom-up into NL, then compare against the original ask}
  \begin{card}
  \tiny
  \textbf{LLM-generated SVA (Stage 2):} \texttt{a \&\& b |-> \#\#2 c}\\
  \textbf{Original ask -- OL-NL (Stage 1):} whenever a and b are both asserted, then c is asserted \textcolor{warnamber}{\textbf{3}} cycles later
  \end{card}
  \vspace{2pt}
  \begin{columns}[T]
    \begin{column}{0.33\textwidth}
      \centering
      {\scriptsize\textbf{1. Parse into a syntax tree}}\\[2pt]
      \resizebox{0.98\textwidth}{!}{%
      \begin{tikzpicture}[
          op/.style={rectangle, rounded corners, draw=nyuvioletdark, fill=nyuvioletdark, text=white, minimum width=1.7cm, minimum height=0.8cm, align=center, font=\scriptsize},
          leaf/.style={rectangle, rounded corners, draw=nyuvioletlight, fill=white, text=cardtext, minimum width=1.2cm, minimum height=0.8cm, align=center, font=\scriptsize},
          arrow/.style={-{Latex[length=2mm]}, thick, nyuvioletdark},
        ]
        \node[op] (imp) at (0,2.4) {\texttt{|->}};
        \node[op] (and) at (-2.0,1.2) {\texttt{\&\&}};
        \node[op] (delay) at (2.0,1.2) {\texttt{\#\#2}};
        \node[leaf] (a) at (-2.9,0) {\texttt{a}};
        \node[leaf] (b) at (-1.1,0) {\texttt{b}};
        \node[leaf] (c) at (2.0,0) {\texttt{c}};

        \draw[arrow] (imp) -- (and);
        \draw[arrow] (imp) -- (delay);
        \draw[arrow] (and) -- (a);
        \draw[arrow] (and) -- (b);
        \draw[arrow] (delay) -- (c);
      \end{tikzpicture}}\\[2pt]
      {\tiny\textcolor{nyumuted}{root \texttt{|->} splits into \texttt{a \&\& b}\\and \texttt{\#\#2 c}; each splits again}}
    \end{column}
    \begin{column}{0.65\textwidth}
      \centering
      {\scriptsize\textbf{2. Merge that same tree bottom-up into NL}}\\[2pt]
      \resizebox{0.98\textwidth}{!}{%
      \begin{tikzpicture}[
          root/.style={rectangle, rounded corners, draw=white, fill=goodgreen!70!nyuvioletdark, text=white, minimum width=8.6cm, minimum height=0.9cm, align=center, font=\scriptsize},
          merge/.style={rectangle, rounded corners, draw=nyuvioletdark, fill=nyuvioletdark, text=white, minimum width=4.0cm, minimum height=0.8cm, align=center, font=\scriptsize},
          leaf/.style={rectangle, rounded corners, draw=nyuvioletlight, fill=white, text=cardtext, minimum width=2.4cm, minimum height=0.8cm, align=center, font=\scriptsize},
          arrow/.style={-{Latex[length=2mm]}, thick, nyuvioletdark},
        ]
        \node[root] (root) at (0,2.4) {whenever a and b are both asserted, then c is asserted 2 cycles later};
        \node[merge] (and) at (-2.2,1.2) {a and b are both asserted};
        \node[merge] (delay) at (2.6,1.2) {c is asserted 2 cycles later};
        \node[leaf] (a) at (-3.4,0) {a is asserted};
        \node[leaf] (b) at (-1.0,0) {b is asserted};
        \node[leaf] (c) at (2.6,0) {c is asserted};
  ...
  \begin{card}
  \tiny
  $\rightarrow$ \textbf{3. Compare with the original ask above -- mismatch found:} the
  tree's root says \textbf{2} cycles later, but the original ask says \textbf{3} --
  SOR flags just the \texttt{\#\#2} node and revises it to \texttt{\#\#3}, not the
  whole property.
  \end{card}
\end{frame}
...
\begin{frame}{Benchmark \& metrics}
  ...
  \begin{itemize}
    \item \textbf{nl2sva\_human\_verified} (73 rows) -- expert-written
      ...
      {\tiny\textit{e.g.} ``that if the current cycle is neither a jump
      operation nor immediately after a reset pulse, the counter must not
      overflow'' $\rightarrow$ \texttt{(!jump\_vld\_d1 \&\& !tb\_reset\_1\_cycle\_pulse\_shadow)
      |-> (count <= max)}}
    \item \textbf{nl2sva\_machine\_verified} (283 rows) -- rule-generated
      ...
      signal\\
      {\tiny\textit{e.g.} ``if both sig\_G and sig\_C are high and sig\_A is
      high, then sig\_I must not be high 5 clock cycles later'' $\rightarrow$
      \texttt{(sig\_G \&\& (sig\_C \&\& sig\_A)) |-> \#\#5 !sig\_I}}
  \end{itemize}
  \end{card}
  \vspace{3pt}
  \begin{card}
  \scriptsize
  \textbf{Comparison work:} \href{https://arxiv.org/abs/2603.14239}{QiMeng-CodeV-SVA:
  Training Specialized LLMs for Hardware Assertion Generation via
  RTL-Grounded Bidirectional Data Synthesis}, DAC 2026 -- a model
  finetuned specifically for NL2SVA; shown alongside general models as a
  raw-output baseline.
  \end{card}
  \vspace{2pt}
  \footnotesize
  \textbf{Metrics} (via a real JasperGold \texttt{prop\_eq\_checker}):
  \textbf{SC} -- does the candidate elaborate against the real testbench
  on its own? \textbf{FM-strict} -- formally proven full equivalence to
  the golden SVA.
\end{frame}

\section{Results: RAG+SOR Pipeline}

\begin{frame}{Results (nl2sva\_human\_verified)}
  \framesubtitle{Our pipeline vs.\ raw model output vs.\ CodeV-SVA}
  \begin{card}
  \centering
  \tiny
  ...
  \begin{tabular}{l l r r r}
    \toprule
    \textbf{Model} & \textbf{Configuration} & \textbf{SC} & \textbf{FM-strict} & \textbf{$\Delta$} \\
    \midrule
    \multirow{2}{*}{gpt-4o} & Raw (0-shot) & 94.5\% & 63.0\% & \\
      & \textbf{+ Our pipeline} & 98.6\% & \textbf{74.0\%} & \textbf{+11.0} \\
    \midrule
    \multirow{2}{*}{DeepSeek-V4-Flash (thinking)} & Raw (0-shot) & 98.6\% & 76.7\% & \\
      & \textbf{+ Our pipeline} & 100.0\% & \textbf{86.3\%} & \textbf{+9.6} \\
    \midrule
    \multirow{2}{*}{deepseek-chat (thinking off)} & Raw (0-shot) & 94.5\% & 54.8\% & \\
      & \textbf{+ Our pipeline} & 100.0\% & \textbf{74.0\%} & \textbf{+19.2} \\
    \midrule
    \multirow{2}{*}{qwen3-8b (Aliyun)} & Raw (base) & 89.0\% & 35.6\% & \\
      & + Our pipeline & 91.8\% & 35.6\% & +0.0 \\
    \midrule
    \multirow{2}{*}{qwen3-14b (Aliyun)} & Raw (base) & 93.2\% & 49.3\% & \\
      & \textbf{+ Our pipeline} & 91.8\% & \textbf{65.8\%} & \textbf{+16.5} \\
    \midrule
    \multirow{2}{*}{gpt-5} & Raw (0-shot) & -- & -- & \\
      & \textbf{+ Our pipeline} & \textbf{100.0\%} & \textbf{82.2\%} & -- \\
    \midrule
  ...
  \end{card}
  \vspace{2pt}
  {\scriptsize\textcolor{nyumuted}{Best per model family in \textbf{bold} (out of 73 rows).
  DeepSeek-V4-Flash + our pipeline (86.3\%) already beats both CodeV-SVA sizes
  (75.3\% / 74.0\%), a model finetuned specifically for NL2SVA.}}
\end{frame}

\begin{frame}{Results (nl2sva\_machine\_verified)}
  \framesubtitle{Our pipeline vs.\ raw model output vs.\ CodeV-SVA}
  \begin{card}
  \centering
  \tiny
  ...
  \begin{tabular}{l l r r r}
    ...
    \midrule
    \multirow{2}{*}{DeepSeek-V4-Flash (thinking)} & Raw (0-shot) & 100.0\% & 90.8\% & \\
      & \textbf{+ Our pipeline} & 100.0\% & \textbf{92.9\%} & \textbf{+2.1} \\
    \midrule
    \multirow{2}{*}{deepseek-chat (thinking off)} & Raw (0-shot) & 97.5\% & 78.4\% & \\
      & \textbf{+ Our pipeline} & 98.9\% & \textbf{85.5\%} & \textbf{+7.1} \\
    \midrule
    \multirow{2}{*}{qwen3-8b (Aliyun)} & Raw (base) & 87.6\% & 51.2\% & \\
      & \textbf{+ Our pipeline} & 88.7\% & \textbf{60.8\%} & \textbf{+9.6} \\
    \midrule
    \multirow{2}{*}{qwen3-14b (Aliyun)} & Raw (base) & 91.9\% & 65.7\% & \\
      & \textbf{+ Our pipeline} & 95.1\% & \textbf{81.6\%} & \textbf{+15.9} \\
    \midrule
  ...
  \end{card}
  \vspace{6pt}
  {\scriptsize\textcolor{nyumuted}{Best per model family in \textbf{bold} (out of 283 rows).
  DeepSeek-V4-Flash + our pipeline (92.9\%) is the strongest result overall, ahead of both CodeV-SVA sizes
  (83.0\% / 80.9\%). gpt-5's machine run was interrupted (45/283) and is omitted.}}
\end{frame}
...
\begin{frame}{Spec2NL + NL2SVA = end-to-end Spec2SVA}
  ...
  \begin{card}
    \footnotesize
    NL2SVA runs with \textbf{the same settings as the
    nl2sva\_machine\_verified evaluation}, minus Step 1 (AssertionForge's
    plans are already signal-grounded) -- plus support for a real design's
    actual clock name (rarely just \texttt{clk}).
  \end{card}
\end{frame}
...
\begin{frame}{Case study: UART}
  ...
  \begin{card}
    \scriptsize
    \textbf{A real gap found \& fixed along the way:} upstream's own
    signal list example only listed 2 of the 18 real signals, and its
    LLM-client code shipped as an empty ``add your own client'' stub --
    both needed fixing before Stage 1/2 ran end to end.
  \end{card}
\end{frame}
