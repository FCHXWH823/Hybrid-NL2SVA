...
if task == 'gen_plan':
    ...
    if subtask == 'actual_gen':
        ...
        if gen_plan_sva_using_valid_signals:
            # valid_signals = None
            valid_signals = ['baud_clk', 'baud_freq', 'baud_limit', 'ce_16', 'int_address', 'int_gnt', 'int_rd_data', 'int_read', 'int_req', 'int_wr_data', 'int_write', 'new_rx_data', 'new_tx_data', 'rx_data', 'ser_in', 'ser_out', 'tx_busy', 'tx_data']
    ...
    # env_source_path/settings_source_path: a GraphRAG `.env` (with your own
    # GRAPHRAG_API_KEY) + settings.yaml, e.g. generated via
    # `python -m graphrag.index --init --root <dir>` and then edited (model
    # -> gpt-4o, snapshots.graphml -> true for the run this pilot used).
    env_source_path = '/<path>/<to>/rag_template/.env'

    settings_source_path = (
        '/<path>/<to>/rag_template/settings.yaml'
    )

    # entity_extraction_prompt_source_path = '/<path>/<to>/rag_apb/prompts/entity_extraction_vanilla_graphRAG.txt'  # original/baseline
    entity_extraction_prompt_source_path = str(Path(__file__).resolve().parents[1] / 'entity_extraction.txt')  # better- customized for HW, vendored alongside this file
...
graphrag_local_dir = '/<path>/<to>/graphrag'  # only needed if NOT pip-installing graphrag (see ../../README.md's
                                               # Reproducing section -- `pip install graphrag==0.3.6` is what the
                                               # actual pilot run used instead)
...
)  # Adjusted: this file now lives at
   # <Hybrid-NL2SVA repo>/end2end_evaluation/AssertionForge/src/config.py, vendored
   # inside Hybrid-NL2SVA's own repo (parents[3] = Hybrid-NL2SVA's root, which has
   # the .git this is actually used to introspect for run-logging metadata --
   # AssertionForge's own upstream .git isn't carried over on purpose, see
   # ../../README.md). A standalone (non-vendored) AssertionForge clone would need
   # parents[1] instead (our clone's repo root == <repo>/src/config.py's parent),
   # not the original authors' <repo>/subproject/src/config.py (parents[2]) layout.
