...
| 16.12.7 | Overlapped implication | `sequence_expr |-> property_expr` | When the antecedent sequence matches, start the consequent in the **same clock cycle as the final cycle of that antecedent match**. For a one-cycle antecedent such as `req`, check `ack` in the same cycle as `req`. If the antecedent has no match, the implication succeeds vacuously. | `assert property (@(posedge clk) req |-> ack);` |
| 16.12.7 | Nonoverlapped implication | `sequence_expr |=> property_expr` | When the antecedent sequence matches, start the consequent in the **next clock cycle after the final cycle of that antecedent match**. For a one-cycle antecedent such as `req`, check `ack` one cycle after `req`. If the antecedent has no match, the implication succeeds vacuously. | `assert property (@(posedge clk) req |=> ack);` |
